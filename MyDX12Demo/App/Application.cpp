...
Microsoft::WRL::ComPtr<IDXGIAdapter4> Application::GetAdapter(bool bUseWarp)
{
    ...
    else
    {
        ...
        for (UINT i = 0; dxgiFactory->EnumAdapters1(i, &dxgiAdapter1) != DXGI_ERROR_NOT_FOUND; ++i)
        {
            ...
            // 检查适配器是否可以在不实际创建 D3D12 设备的情况下创建它
            // 具有最大专用视频内存的适配器受到青睐
...
bool Application::CheckTearingSupport()
{
    ...
    // 我们不是直接创建 DXGI 1.5 工厂接口，而是创建 DXGI 1.4 接口并查询 1.5 接口。
    // 这是为了启用图形调试工具，这些工具在将来的更新之前将不支持 1.5 出厂界面。
...
DXGI_SAMPLE_DESC Application::GetMultisampleQualityLevels( DXGI_FORMAT format, UINT numSamples, D3D12_MULTISAMPLE_QUALITY_LEVEL_FLAGS flags ) const
{
    ...
    while ( qualityLevels.SampleCount <= numSamples
        && SUCCEEDED( m_d3d12Device->CheckFeatureSupport( D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS,
            &qualityLevels,sizeof( D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS ) ) )
        && qualityLevels.NumQualityLevels > 0 )
    {
